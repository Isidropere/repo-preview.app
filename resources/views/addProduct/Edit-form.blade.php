...
<div class="min-h-screen bg-gray-50 py-5">
    <div class="max-w-xl mx-auto px-4">
        ...
        <div class="text-center mb-5">
            ...
            <p class="text-gray-500 mt-1">Actualiza la información de tu artículo</p>
        </div>
                        ...
                        <div>
                            ...
                            <select id="id_categoria_item" name="id_categoria_item" required
                                ...
                                @foreach($categorias as $categoria)
                                    <option value="{{ $categoria->id_categoria_item }}" {{ old('id_categoria_item', $item->id_categoria_item) == $categoria->id_categoria_item ? 'selected' : '' }}>{{ $categoria->categoria }}</option>
                                @endforeach
                            </select>
                ...
                <div class="space-y-2.5">
                    ...
                    <div>
                        ...
                        <div class="flex justify-between items-center mt-0.5">
                            ...
                            <span id="contadorProducto" class="text-xs text-gray-400 ml-auto">{{ strlen(old('presentacion', $item->presentacion ?? '')) }}/250</span>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select id="estatus" name="estatus" class="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors bg-white">
                                ...
                                <option value="3" {{ old('estatus', $item->estatus) == 3 ? 'selected' : '' }}>Pendiente</option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div id="dimensions-panel" class="px-4 py-4 space-y-3">
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                <div>
                                    ...
                                    <input type="number" id="peso_lbs" name="peso_lbs" step="0.01" min="0.01" value="{{ old('peso_lbs', $item->peso_lbs ?: 1) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
                                </div>
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">Alto (cm) *</label>
                                    <input type="number" id="alto_cm" name="alto_cm" step="0.1" min="0.1" value="{{ old('alto_cm', $item->alto_cm ?: 1) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
                                </div>
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">Ancho (cm) *</label>
                                    <input type="number" id="ancho_cm" name="ancho_cm" step="0.1" min="0.1" value="{{ old('ancho_cm', $item->ancho_cm ?: 1) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
                                </div>
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">Profundidad (cm) *</label>
                                    <input type="number" id="profundo_cm" name="profundo_cm" step="0.1" min="0.1" value="{{ old('profundo_cm', $item->profundo_cm ?: 1) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
                                </div>
                            </div>
                                        ...
                                        <div id="family-{{ Str::slug($family) }}" class="hidden space-y-1 ml-2">
                                            ...
                                            <div class="flex items-center justify-between py-1.5 px-2 hover:bg-gray-50 rounded">
                                                <label for="color-{{ $color->id_color }}" class="flex items-center cursor-pointer">
                                                    <input type="checkbox" id="color-{{ $color->id_color }}" name="colors[]" value="{{ $color->id_color }}" class="color-checkbox mr-2 h-4 w-4 text-primary rounded focus:ring-primary" {{ in_array($color->id_color, old('colors', $selectedColors)) ? 'checked' : '' }}>
                                                ...
                                                <input type="number" name="stock[{{ $color->id_color }}]" min="0" placeholder="Stock" value="{{ old('stock.'.$color->id_color, $stockByColor[$color->id_color] ?? 0) }}"
                                                       class="stock-input text-sm w-16 px-2 py-1 border border-gray-300 rounded focus:ring-1 focus:ring-primary {{ in_array($color->id_color, old('colors', $selectedColors)) ? '' : 'hidden' }}">
                                            </div>
                                            @endforeach
                                        </div>
                                    </div>
                            ...
                            <div id="selectedColorsPreview" class="mt-3 flex flex-wrap gap-2">
                                @foreach($colors as $color)
                                    ...
                                    <div class="flex items-center bg-gray-100 rounded-full px-3 py-1" data-colorid="{{ $color->id_color }}">
                                        ...
                                        <span class="ml-2 text-xs bg-gray-200 px-2 py-0.5 rounded-full">Stock: {{ old('stock.'.$color->id_color, $stockByColor[$color->id_color] ?? 0) }}</span>
...
function validarPaso2Producto() {
    // En edición, la imagen principal ya puede existir en el servidor
    const inp = document.getElementById('imagen_principal');
    const prevImg = document.getElementById('imagen_principal_preview');
    const hasExisting = prevImg && prevImg.src && !prevImg.classList.contains('hidden');
    const hasNew = inp && inp.files && inp.files.length > 0;

    if (!hasExisting && !hasNew) {
        const contenedor = inp ? inp.closest('label') : null;
    ...
    const msgEl = document.getElementById('error-imagen-principal');
...
// Abrir dimensiones si ya tiene valores
@if($item->peso_lbs > 0 || $item->alto_cm > 0 || $item->ancho_cm > 0 || $item->profundo_cm > 0)
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('dimensions-panel').classList.remove('hidden');
    document.getElementById('dim-arrow').classList.add('rotate-180');
});
@endif
...
document.querySelectorAll('.group-toggle').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        if (target) { target.classList.toggle('hidden'); this.querySelector('.group-arrow').classList.toggle('rotate-180'); }
    });
});

document.querySelectorAll('.color-checkbox').forEach(cb => {
    cb.addEventListener('change', function() {
        const stockInput = this.closest('div.flex').querySelector('.stock-input');
        if (stockInput) stockInput.classList.toggle('hidden', !this.checked);
        updateColorPreview();
    });
});
...
function updateColorPreview() {
    ...
    document.querySelectorAll('.color-checkbox:checked').forEach(cb => {
        const label = cb.closest('div.flex').querySelector('label');
        const hex = label.querySelector('span[style]').style.backgroundColor;
        const name = label.querySelector('span.text-sm').textContent;
        const stock = cb.closest('div.flex').querySelector('.stock-input')?.value || 0;
        const div = document.createElement('div');
        div.className = 'flex items-center bg-gray-100 rounded-full px-3 py-1';
        div.dataset.colorid = cb.value;
        div.innerHTML = `<span class="w-3 h-3 rounded-full mr-1" style="background-color:${hex}"></span><span class="text-xs">${name}</span><span class="ml-2 text-xs bg-gray-200 px-2 py-0.5 rounded-full">Stock: ${stock}</span>`;
