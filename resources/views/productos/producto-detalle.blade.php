...
@php
    ...
    $imagenes     = $item->imagenes->where('estado', 'aprobado')->count() > 0 
                    ? $item->imagenes->where('estado', 'aprobado')->sortBy('orden_visualizacion')
                    : $item->imagenes->sortBy('orden_visualizacion');
...
<div style="max-width:1140px;margin:0 auto;padding:0 1.25rem;">
    ...
    <div style="background:#fff;border-radius:1.1rem;border:1px solid #e2e8f0;box-shadow:0 1px 3px rgba(0,0,0,.04);overflow:hidden;">
        ...
        <div style="padding:1rem;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0.75rem;" class="sm:grid-cols-related-products">
            ...
            @php
                $ri  = $rel->imagenes->where('estado', 'aprobado')->first() ?? $rel->imagenes->first();
            ...
            <a href="{{ route('producto.detalle', $rel->slug) }}"
               ...
               onmouseout="this.style.boxShadow='none';this.style.transform='none'">
                <div style="height:76px;overflow:hidden;background:#f1f5f9;">
                    @if($riu)
                    <img src="{{ $riu }}" alt="{{ $rel->item }}" style="width:100%;height:100%;object-fit:cover;" loading="lazy" width="200" height="76" onerror="this.src='/imgs/defaults/producto_default.svg'">
                    @else
...
{{-- MODAL ZOOM --}}
<div id="zoomModal" onclick="closeZoom()" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,0.45);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);z-index:9999;overflow-x:auto;overflow-y:hidden;align-items:center;justify-content:flex-start;cursor:zoom-out;user-select:none;scroll-snap-type:x mandatory;">
