...
@section('title', 'Cambialord - Mis Productos')
...
<main class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-5xl mx-auto px-4">
        @auth
        <div class="mb-4">
            <a href="{{ route('tu_cuenta') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-primary">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Gestiona tus artículos publicados</p>
            </div>
            <a href="{{ route('items.create') }}" class="inline-flex items-center gap-2 bg-primary hover:bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm transition-all">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3">
                <div class="flex-1 relative">
                    ...
                    <input type="text" id="searchInput" placeholder="Buscar producto..." class="w-full pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary bg-gray-50" style="padding-left:2.5rem;">
                </div>
            ...
            <div class="divide-y divide-gray-100">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="w-16 h-16 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0 cursor-pointer border border-gray-200" onclick="abrirImagen(this.querySelector('img')?.src)">
                            @if($item->todasLasImagenes->isNotEmpty())
                                ...
                                @endphp
                                <img src="{{ $imageUrl }}" alt="{{ $item->item }}" class="w-full h-full object-cover" loading="lazy" width="200" height="200" onerror="this.onerror=null;this.src='{{ asset('imgs/defaults/producto_default.svg') }}'">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-x-4 gap-y-0.5 text-xs text-gray-500">
                                ...
                                <span>{{ $item->tipo_trans == 1 ? 'Venta' : ($item->tipo_trans == 2 ? 'Intercambio' : 'Ambos') }}</span>
                        ...
                        <div class="flex items-center gap-1.5 flex-shrink-0">
                            ...
                            <a href="{{ route('items.edit', $item->slug) }}" class="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Editar">
                ...
                <div class="p-12 text-center">
                    ...
                    <h3 class="text-base font-semibold text-gray-700">No tienes productos</h3>
                    <p class="text-sm text-gray-400 mt-1 mb-4">Comienza publicando tu primer artículo</p>
                    <a href="{{ route('items.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-blue-600">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                        Nuevo Producto
                    </a>
                </div>
...
function confirmDelete(btn) { if (confirm('¿Eliminar este producto?')) btn.closest('form').submit(); }
