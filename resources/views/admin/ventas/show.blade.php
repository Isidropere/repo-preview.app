...
@include('components.btn-volver', ['backUrl' => route('admin.index', ['tab' => 'ventas'])])
    ...
    <div id="mainContent" class="hidden">
        ...
        @php
            ...
            $ventador = $venta->comprador;

            $txtOrden  = "Orden #{$venta->id_pago_compra}\n";
            $txtOrden .= "Comprador: " . ($ventador?->nombres ?? '') . " " . ($ventador?->apellidos ?? '') . "\n";
            $txtOrden .= "Email: " . ($ventador?->email ?? 'N/A') . "\n";
            $txtOrden .= "Fecha: " . ($venta->fecha ? \Carbon\Carbon::parse($venta->fecha)->format('d/m/Y H:i') : 'N/A') . "\n";
            $txtOrden .= "Estado: " . ucfirst($venta->estatus ?? '') . "\n";
            $txtOrden .= "Total: RD$ " . number_format($venta->total ?? 0, 2) . "\n";
            if ($venta->direccion) {
                $d = $venta->direccion;
                $txtOrden .= "\nDirección de envío:\n";
            ...
            $emailSubject = "Envío - Orden #{$venta->id_pago_compra}";
                ...
                <div class="space-y-1">
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl font-bold text-gray-900">
                            Orden <span class="font-mono text-base">#{{ $venta->id_pago_compra }}</span>
                        </h1>
                        <button type="button" onclick="previewPdf('{{ route('admin.compras.pdf', $venta->id_pago_compra) }}')"
                    ...
                    <div class="flex items-center gap-2 text-sm text-gray-700">
                        ...
                        <span class="font-medium">{{ $ventador->nombres }} {{ $ventador->apellidos ?? '' }}</span>
                        <span class="text-gray-400">&mdash;</span>
                        <span class="text-gray-500">{{ $ventador->email }}</span>
                    </div>
                    @else
                    ...
                    <div class="flex flex-wrap items-center gap-3 text-xs text-gray-400">
                        ...
                        @endif
                        @if($venta->total)
                        <span class="font-semibold text-gray-700 text-sm">RD$ {{ number_format($venta->total, 2) }}</span>
                        @endif
                        @if($venta->cantidad_items)
                        <span>{{ $venta->cantidad_items }} artículo(s)</span>
                        @endif
                    </div>
                </div>
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                        ...
                        @if($venta->pagoItems->count())
                        <span class="text-xs text-gray-400">{{ $venta->pagoItems->count() }} artículo(s)</span>
                        @elseif($venta->cantidad_items)
                        <span class="text-xs text-gray-400">{{ $venta->cantidad_items }} artículo(s)</span>
                        @endif
                    </div>
                    ...
                    <div class="px-5 py-3 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                        ...
                        <span class="text-lg font-bold text-gray-900">
                            RD$ {{ number_format($venta->total ?? $venta->pagoItems->sum('subtotal'), 2) }}
                        </span>
                    </div>
                    @else
                    <div class="px-5 py-10 text-center text-gray-400 text-sm">
                        ...
                        @if($venta->cantidad_items)
                        <p>{{ $venta->cantidad_items }} artículo(s) procesados</p>
                ...
                @php
                    $txtArticulos = "Artículos — Orden #{$venta->id_pago_compra}\n";
                    foreach ($venta->pagoItems as $pi) {
                    ...
                    $txtArticulos .= "Total: RD$ " . number_format($venta->total ?? $venta->pagoItems->sum('subtotal'), 2);
                @endphp
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <dd class="font-medium text-gray-700">{{ $venta->proveedorPago?->proveedor_pago ?? 'N/A' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs text-gray-400 uppercase tracking-wide mb-1">Código de autorización</dt>
                            <dd class="font-mono font-medium text-gray-700">{{ $venta->autorizacion_pago ?? 'N/A' }}</dd>
                        </div>
                        @if($venta->transaction_id)
                        <div class="sm:col-span-2">
                            <dt class="text-xs text-gray-400 uppercase tracking-wide mb-1">ID de transacción</dt>
                            <dd class="font-mono text-xs text-gray-600 break-all">{{ $venta->transaction_id }}</dd>
                        </div>
                        @endif
                        @if($venta->tarjeta)
                        <div>
                            ...
                            <dd class="font-medium text-gray-700 flex items-center gap-2">
                                ...
                                <span class="px-2 py-0.5 bg-gray-100 rounded text-xs font-semibold uppercase">{{ $venta->tarjeta->tipo_tarjeta }}</span>
                                @endif
                                @if($venta->tarjeta->last4)
                                <span>&bull;&bull;&bull;&bull; {{ $venta->tarjeta->last4 }}</span>
                                @endif
                            </dd>
                        </div>
                        <div>
                            <dt class="text-xs text-gray-400 uppercase tracking-wide mb-1">Titular</dt>
                            <dd class="font-medium text-gray-700">{{ $venta->tarjeta->nombre_titular ?? 'N/A' }}</dd>
                        </div>
                        @if($venta->tarjeta->banco_tarjeta)
                        <div>
                            <dt class="text-xs text-gray-400 uppercase tracking-wide mb-1">Banco</dt>
                            <dd class="font-medium text-gray-700">{{ $venta->tarjeta->banco_tarjeta }}</dd>
                        </div>
                        @endif
                        <div>
                            ...
                            <dd class="font-medium text-gray-700">
                                @php
                                    $mes  = $venta->tarjeta->mes_expiracion;
                                    $anio = $venta->tarjeta->getAttribute(\App\Models\TarjetaPago::COL_ANIO);
                                @endphp
                ...
                {{-- Datos ocultos pago --}}
                @php
                    $txtPago  = "Información de pago — Orden #{$venta->id_pago_compra}\n";
                    $txtPago .= "Proveedor: " . ($venta->proveedorPago?->proveedor_pago ?? 'N/A') . "\n";
                    $txtPago .= "Autorización: " . ($venta->autorizacion_pago ?? 'N/A') . "\n";
                    if ($venta->transaction_id) {
                        $txtPago .= "ID Transacción: {$venta->transaction_id}\n";
                    }
                    if ($venta->tarjeta) {
                        $txtPago .= "Tarjeta: " . ($venta->tarjeta->tipo_tarjeta ?? '') . " **** " . ($venta->tarjeta->last4 ?? '') . "\n";
                        $txtPago .= "Titular: " . ($venta->tarjeta->nombre_titular ?? 'N/A') . "\n";
                        if ($venta->tarjeta->banco_tarjeta) {
                            $txtPago .= "Banco: {$venta->tarjeta->banco_tarjeta}\n";
                        }
                    }
                @endphp
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <dd class="font-medium text-gray-700">{{ $ventador->nombres }} {{ $ventador->apellidos ?? '' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs text-gray-400 uppercase tracking-wide mb-1">Email</dt>
                            <dd class="font-medium text-gray-700">{{ $ventador->email }}</dd>
                        </div>
                        @if($ventador->telefono ?? null)
                        <div>
                            <dt class="text-xs text-gray-400 uppercase tracking-wide mb-1">Teléfono</dt>
                            <dd class="font-medium text-gray-700">{{ $ventador->telefono }}</dd>
                        </div>
                        @endif
                    </dl>
                </div>
                @php
                    $txtComprador  = "Comprador — Orden #{$venta->id_pago_compra}\n";
                    $txtComprador .= "Nombre: {$ventador->nombres} " . ($ventador->apellidos ?? '') . "\n";
                    $txtComprador .= "Email: {$ventador->email}\n";
                    if ($ventador->telefono ?? null) {
                        $txtComprador .= "Teléfono: {$ventador->telefono}\n";
                    }
                @endphp
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    ...
                    @php
                        $primerItem = $venta->pagoItems->first();
                        $dirRec = null;
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            @if($venta->direccion)
                            @php $dir = $venta->direccion; @endphp
                            <dl class="text-sm space-y-2">
                                <div>
                                    <dt class="text-xs text-gray-400 uppercase tracking-wide">Calle / Dirección</dt>
        ...
        <form method="POST" action="{{ route('admin.compras.tracking', $venta->id_pago_compra) }}"
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <p class="text-xs text-gray-400">
                    ...
                    <span class="font-medium text-gray-600">{{ $venta->comprador->nombres }} ({{ $venta->comprador->email }})</span>.
                </p>
                @endif
            </div>
...
<div id="modalNotificacion" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form method="POST" action="{{ url('/admin/notificaciones/enviar') }}">
            ...
            <input type="hidden" name="usuario_id" value="{{ $ventador->id }}">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div class="p-3 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <p class="text-sm font-semibold text-gray-800">{{ $ventador->nombres }} {{ $ventador->apellidos ?? '' }}</p>
                    <p class="text-xs text-gray-500">{{ $ventador->email }}</p>
                </div>
