...
@section('title', 'Nuevo Motivo de Devolución')
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Crear Nuevo Motivo</h1>
            <p class="text-sm text-gray-500 mt-1">Registra un nuevo motivo que los usuarios podrán seleccionar al realizar devoluciones.</p>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-6">
            <form action="{{ route('admin.motivos_devolucion.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="activo" id="activo" class="border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent w-full">
                        ...
                        <option value="0" {{ old('activo') == '0' ? 'selected' : '' }}>Inactivo (Ocultar temporalmente)</option>
                    </select>
                    @error('activo')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
