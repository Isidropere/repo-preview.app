...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 mt-1">Modifica los detalles de la oferta de trabajo seleccionada.</p>
        </div>

        <form action="{{ route('admin.recursos-humanos.update', $empleo->id) }}" method="POST" class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="titulo" class="block text-xs font-semibold text-gray-600 mb-1.5">Título del Puesto / Vacante</label>
                <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $empleo->titulo) }}" required placeholder="Ej: Desarrollador Backend Laravel, Cajero, Asistente de Operaciones..."
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" id="descripcion" rows="6" required placeholder="Describe las responsabilidades del puesto, la misión del rol, etc..."
                          class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary">{{ old('descripcion', $empleo->descripcion) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <textarea name="requisitos" id="requisitos" rows="6" required placeholder="Requisitos del puesto y detalles de cómo postularse (Ej: 'Enviar CV a cambialord.com@gmail.com con el asunto...')"
                          class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary">{{ old('requisitos', $empleo->requisitos) }}</textarea>
            ...
            <div class="mb-6">
                <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="activo" value="1" {{ old('activo', $empleo->activo) ? 'checked' : '' }}
                           class="w-4.5 h-4.5 border border-gray-200 rounded text-primary focus:ring-primary">
                    <span class="text-xs font-semibold text-gray-600">Vacante Activa (Visible en el portal)</span>
                </label>
            </div>
