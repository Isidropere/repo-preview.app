...
<div class="flex flex-col items-center justify-center py-20 text-gray-400">
    ...
    <p class="text-sm font-medium">No se encontraron intercambios</p>
</div>
@else
<div class="overflow-x-auto">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            @php
                $imagen = $intercambio->item?->imagenes?->first();
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="font-medium text-gray-800 truncate max-w-[140px]">{{ $intercambio->item?->item ?? 'Sin artículo' }}</span>
                    </div>
                </td>
                <td class="px-4 py-3">
                    @if($intercambio->usuario)
                        <p class="font-medium text-gray-800">{{ $intercambio->usuario->nombres }}</p>
                        <p class="text-xs text-gray-400">{{ $intercambio->usuario->email }}</p>
                    @else
                ...
                <td class="px-4 py-3">
                    @if($intercambio->usuarioReceptor)
                        <p class="font-medium text-gray-800">{{ $intercambio->usuarioReceptor->nombres }}</p>
                        <p class="text-xs text-gray-400">{{ $intercambio->usuarioReceptor->email }}</p>
                    @else
                        <span class="text-gray-400 text-xs">Sin datos</span>
                    @endif
                </td>
...
<div class="px-4 py-3 border-t border-gray-100">
    {{ $intercambios->appends(request()->except('page'))->links('vendor.pagination.custom') }}
</div>
@endif
@endif
